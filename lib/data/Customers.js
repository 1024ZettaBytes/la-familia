...
import { validateMapsUrl, getCoordinatesFromUrl, dateDiffInWeeks, setDateToMid } from "../client/utils";
...
import { MachineStatus } from "lib/models/MachineStatus";
Machine.init();
...
export async function saveCustomerData({
  ...
  try {
    ...
    let existingMachine = await Machine.findOne({ machineNum: machineNumber });
    if (!existingMachine) {
      error.message = "No existe un equipo con el número " + machineNumber;
      throw error;
    }
   const machineStatus = await MachineStatus.findOne({id: "RENTADO"});
   existingMachine.status = machineStatus._id;

    console.log(machineNumber)
    console.log(startDate)
    const totalRentWeeks = dateDiffInWeeks(new Date(endDate), new Date(startDate));
    console.log(endDate)
    console.log(totalRentWeeks)
    ...
    if(totalRentWeeks>=8)
    cLevelId = "permanente";
  const cLevel = await CustomerLevel.findOne({id:cLevelId});
    ...
    let newCustomerData = {
      ...
      updatedAt: new Date(startDate),
      totalRentWeeks,
      firstRentAt: setDateToMid(new Date()), 
      hasRent: true,
      lastUpdatedBy,
    };
    ...
    let newCustomer = await new Customer({ ...newCustomerData }).save({
      session,
      new: true,
    });
    if (referredByCustomer) {
    ...
    const rentStatus = await RentStatus.findOne({ id: "RENTADO" });
    let rent = await new Rent({
      ...
      usedFreeWeeks: 0,
      initialWeeks: totalRentWeeks,
      initialPay: 0,
      createdAt: new Date(startDate),
      updatedAt: new Date(startDate),
      createdBy: lastUpdatedBy,
      lastUpdatedBy,
      totalWeeks: totalRentWeeks,
      startDate: new Date(startDate),
      endDate: new Date(endDate)
    }).save({ session, new: true });

    existingMachine.lastRent = rent._id;
    await existingMachine.save({session, new: false});
    newCustomer.currentRent =  rent._id;
    await newCustomer.save({session, new:false});
    ////
