...
export async function saveCustomerData({
  ...
  try {
    ...
    if (existingMachine.status.id === "RENTADO") {
      error.message = "El equipo ya tiene una renta activa";
      throw error;
    }
    ...
    const totalRentWeeks = dateDiffInWeeks(
      new Date(endDate),
      new Date(startDate)
    );
    console.log(endDate);
    ...
    let cLevelId = "nuevo";
    if (totalRentWeeks >= 4) cLevelId = "regular";
    if (totalRentWeeks >= 8) cLevelId = "permanente";
