...
export async function getSummaryByRange(start, end) {
  ...
  // CASH Payments
  let defaultDaysCash = structuredClone(defaultDays);
  const newCashPayments = await Payment.find({
    method: "CASH",
    date: { $gte: startDate, $lte: endDate },
  });

  for (const i in newCashPayments) {
    const payment = newCashPayments[i];
    const formatted = format(payment?.date, "dd/MM/yyyy");
    defaultDaysCash[formatted] = {
      ...defaultDaysCash[formatted],
      done: defaultDaysCash[formatted].done + 1,
    };
  }
  result.cashPayments.days = Object.values(defaultDaysCash);
  result.cashPayments.totalData.done = newCashPayments.length;
  return result;
}
